...
def read_projection_result(output_dir):
    ...
    outputs = pl.read_csv(output_dir / "projection" / "all_simulations.csv")
    return (
...
def test_projection_griddle_matches_dataframe(tmp_path, base_config, model_binary):
    ...
    ProjectionContext(projection_config(base_config, model_binary), griddle_output).run(
        particles=particles_from_griddle
    )

    dataframe_output = tmp_path / "dataframe"
    ProjectionContext(
        projection_config(base_config, model_binary), dataframe_output
    ).run(particles=particles_from_dataframe)
